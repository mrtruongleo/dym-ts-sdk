    ...
    toProto() {
        ...
        if (params.spotOrdersToCreate && params.spotOrdersToCreate.length > 0) {
            const orderData = params.spotOrdersToCreate.map((args) => {
                ...
                const paramsFromArgs = Object.assign(Object.assign({}, args), { price: amountToCosmosSdkDecAmount(args.price).toFixed(), triggerPrice: amountToCosmosSdkDecAmount(args.triggerPrice || 0).toFixed(), quantity: amountToCosmosSdkDecAmount(args.quantity).toFixed() });
        ...
        if (params.derivativeOrdersToCreate &&
            ...
            const orderData = params.derivativeOrdersToCreate.map((args) => {
                const paramsFromArgs = Object.assign(Object.assign({}, args), { price: amountToCosmosSdkDecAmount(args.price).toFixed(), margin: amountToCosmosSdkDecAmount(args.margin).toFixed(), triggerPrice: amountToCosmosSdkDecAmount(args.triggerPrice || 0).toFixed(), quantity: amountToCosmosSdkDecAmount(args.quantity).toFixed() });
                const orderInfo = InjectiveExchangeV1Beta1Exchange.OrderInfo.create();
                orderInfo.subaccountId = params.subaccountId;
                orderInfo.feeRecipient = paramsFromArgs.feeRecipient;
                orderInfo.price = paramsFromArgs.price;
